...
- 可重复读：一个事务执行过程中看到的数据总是和事务开始时看到的数据是一致的，**MySQL默认隔离级别**，可以杜绝幻读。
- 串行化：对读写操作都加锁
...
- 可重复读的核心是一致性读；而事务更新数据时是当前读，如果当前读的记录的行锁被其他事务占用的话，只能进入锁等待
- A事务对一条数据update时，B事务先对这条数据做了update，由于更新数据是当前读，会占用行锁，所以A事务进入锁等待，直到B事务commit
